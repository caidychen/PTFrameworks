...
- (void)adjustFocusView {
    ...
    //避免得到一个超出范围的值
    if(!CGRectContainsPoint(self.recorder.previewView.bounds, viewPoint)) {
        viewPoint = CGPointMake(CGRectGetWidth(self.recorder.previewView.bounds) / 2.0f, CGRectGetHeight(self.recorder.previewView.bounds) / 2.0f);
    }
