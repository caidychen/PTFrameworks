...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        if (!view.superview) {
            ...
            //fix iOS7 UITableViewWrapperView bug
            CGFloat sysVersion = [[[UIDevice currentDevice] systemVersion] floatValue];
            if(sysVersion >= 7.0f && sysVersion < 8.0f) {
                [self addSubview:view];
            } else if (([self isKindOfClass:[UITableView class]] || [self isKindOfClass:[UICollectionView class]]) && self.subviews.count > 1) {
