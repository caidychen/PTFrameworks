...
BOOL PPTISDictionaryAndNotNil(NSDictionary *dict) {
    return (dict && [dict isKindOfClass:[NSDictionary class]]);
}


@interface PPTKillErrorObj : NSObject
@end

@implementation PPTKillErrorObj
@end
static PPTKillErrorObj *killErrorObj = nil;
PPTKillErrorObj * PPTSharedKillErrorObj() {
    if(!killErrorObj) {
        killErrorObj = [[PPTKillErrorObj alloc] init];
    }
    return (killErrorObj);
}


@interface PPTBaseItem () {
...
+ (instancetype)itemWithDict:(NSDictionary *)dict {
    id item = [self item];
    if(!PPTISDictionaryAndNotNil(dict)) {
        return (item);
    }
...
- (id)copyWithZone:(NSZone *)zone {
    PPTBaseItem *item = [[[self class] allocWithZone:zone] init];
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector {
    ...
    if (!signature) {
        PPTKillErrorObj *killErrorObj = PPTSharedKillErrorObj();
