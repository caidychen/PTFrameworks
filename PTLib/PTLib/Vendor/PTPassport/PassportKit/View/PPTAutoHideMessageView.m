...
CGFloat const PPTAutoHideMessageDefaultHideDelay        = 1.0f;
static void * PPT_msgLabelKey = &PPT_msgLabelKey;


@implementation PPTAutoHideMessageView
...
+ (void)showMessage:(NSString *)msg inView:(UIView *)view positionOffset:(CGPoint)offset {
    [self showMessage:msg inView:view positionOffset:offset hideDelay:PPTAutoHideMessageDefaultHideDelay];
}
...
+ (void)showMessage:(NSString *)msg inView:(UIView *)view positionOffset:(CGPoint)offset hideDelay:(NSTimeInterval)delay {
    if(view && msg && delay > 0) {
        UILabel *lb = (UILabel *)objc_getAssociatedObject(view, PPT_msgLabelKey);
        if(!lb) {
            ...
            [view addSubview:lb];
            objc_setAssociatedObject(view, PPT_msgLabelKey, lb, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            
            __PPTWEAK typeof(lb) weak_lb = lb;
            double delayInSeconds = delay;
            dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
            dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                [weak_lb removeFromSuperview];
                objc_setAssociatedObject(view, PPT_msgLabelKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            });
        }
