...
@implementation PTUploadAssetHelper


+ (PTUploadAssetHelper *)sharedAssetHelper
{
    static PTUploadAssetHelper *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[PTUploadAssetHelper alloc] init];
...
- (UIImage *)getImageFromAsset:(ALAsset *)asset type:(NSInteger)nType
{
    ...
    if (nType == UPLOAD_ASSET_PHOTO_THUMBNAIL) {
        iRef = [asset thumbnail];
    } else if (nType == UPLOAD_ASSET_PHOTO_ASPECT_THUMBNAIL) {
        iRef = [asset aspectRatioThumbnail];
    } else if (nType == UPLOAD_ASSET_PHOTO_SCREEN_SIZE) {
        iRef = [asset.defaultRepresentation fullScreenImage];
    } else if (nType == UPLOAD_ASSET_PHOTO_FULL_RESOLUTION) {
